...
var webIMEmotionsHt = new Hashtable();
    ...
    $.ajax( {
        ...
        success : function(response) {
            ...
            for ( var i in data) {
                ...
                webIMEmotionsHt.put(data[i].phrase, data[i].icon);
            }
        }
    });
});
...
function AnalyticEmotion(s) {
    if(typeof (s) != "undefined") {
        s = s.replace(/\r/gi,'').replace(/\n/gi,'<br>');
        var sArr = s.match(/\[.*?\]/g);
        if (!sArr) return s;
        for(var i = 0; i < sArr.length; i++){
            if(webIMEmotionsHt.containsKey(sArr[i])) {
                var reStr = "<img src=\"" + webIMEmotionsHt.get(sArr[i]) + "\" height=\"22\" width=\"22\" />";
...
(function($){
    $.fn.webIMEmotion = function(target){
        var cat_current;
        var cat_page;
        $(this).click(function(event){
            event.stopPropagation();

            var eTop = target.offset().top + target.height() + 15 - 90;
            ...
            $('body').append('<div id="emotions" class="webIM-face clearfix"></div>');
            $('#emotions').css({top: eTop, left: eLeft});
            ...
            $('#emotions').html('<div style="float:right"><a href="javascript:void(0);" id="prev">&laquo;</a><a href="javascript:void(0);" id="next">&raquo;</a></div><div class="categorys"></div><div class="facecontainer"></div><div class="page"></div>');
