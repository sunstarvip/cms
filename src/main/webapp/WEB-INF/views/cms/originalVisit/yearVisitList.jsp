...
<xqlc:extends name="javascript">
    <script type="text/javascript">
        function checkOperate(siteId){
            $('#opereateIframe').attr('src','');
            $('#operateModal').modal();
        }
        $(function(){
            $('#startTime_field').change(function(){
                var sTime = $('input[name = "createTime_ge"]').prop('value');
                var eTime = $('input[name = "createTime_le"]').prop('value');
                alert(sTime+'-'+eTime);
                if(sTime != '' && eTime != ''){
                    if(sTime > eTime){
                        alert('开始时间必须小于结束时间！');
...
<xqlc:extends name="body">
    <div class="appcontent">
        <h3>
            <span style="font-size: x-large;">年统计</span>
        </h3>
    </div>
